...
# =========================
# Smile réel — Données de marché
# =========================
st.markdown('<div class="trading-divider"></div>', unsafe_allow_html=True)
st.subheader("🌍 Volatility Smile — Données de marché")
...
with smile_col1:
    smile_ticker = st.text_input("Ticker", value="AAPL")

# Charger la chaîne d'options
@st.cache_data
def get_option_chain(ticker, expiry=None):
    return load_option_chain(ticker, expiry)

chain_data = get_option_chain(smile_ticker)

if chain_data["expiry"] is None:
    st.error("Aucune option disponible pour ce ticker.")
else:
    with smile_col2:
        selected_expiry = st.selectbox(
            "Expiration",
            chain_data["available_expiries"],
        )

    # Recharger si l'expiration a changé
    if selected_expiry != chain_data["expiry"]:
        chain_data = get_option_chain(smile_ticker, selected_expiry)

    spot_market = chain_data["spot"]
    calls_df = chain_data["calls"]
    puts_df = chain_data["puts"]

    st.markdown(f"**Spot actuel : {spot_market:.2f}** — Expiration : **{selected_expiry}**")

    # Calcul du temps jusqu'à maturité
    from datetime import datetime
    expiry_date = datetime.strptime(selected_expiry, "%Y-%m-%d")
    T_market = max((expiry_date - datetime.now()).days / 365, 0.001)

    # Taux sans risque (on réutilise celui du sidebar)
    r_market = r

    # --- Calcul de la vol implicite pour chaque strike ---
    # On utilise le mid price = (bid + ask) / 2 pour un prix plus juste
    # que le lastPrice qui peut être décalé
    ...
    for _, row in calls_df.iterrows():
        ...
        # On ne garde que les options avec un bid/ask raisonnable
        if bid <= 0 or ask <= 0:
            continue
        ...
        iv_val = implied_volatility(
            mid_price, spot_market, strike_k, T_market, r_market, "call"
        )

        if not np.isnan(iv_val) and 0.01 < iv_val < 3.0:
            iv_data.append({
                "strike": strike_k,
                "iv": iv_val,
                "type": "Call",
    ...
    for _, row in puts_df.iterrows():
        ...
        iv_val = implied_volatility(
            mid_price, spot_market, strike_k, T_market, r_market, "put"
        )
    ...
    if len(iv_data) == 0:
        st.warning("Pas assez de données pour construire le smile.")
    else:
        iv_df = pd.DataFrame(iv_data)

        # --- Graphique du smile ---
        fig_real_smile = go.Figure()

        calls_iv = iv_df[iv_df["type"] == "Call"]
        puts_iv = iv_df[iv_df["type"] == "Put"]

        fig_real_smile.add_trace(go.Scatter(
        ...
        fig_real_smile.add_vline(
            x=spot_market, line_dash="dot", line_color="#f59e0b",
            annotation_text=f"Spot={spot_market:.2f}",
        )

        fig_real_smile.update_layout(
            title=f"Volatility Smile — {smile_ticker} (exp: {selected_expiry})",
            xaxis_title="Strike",
            yaxis_title="Vol implicite",
            yaxis_tickformat=".1%",
            template="plotly_dark",
        ...
        st.plotly_chart(fig_real_smile, use_container_width=True)

        # --- KPIs du smile ---
        atm_calls = calls_iv.iloc[(calls_iv["strike"] - spot_market).abs().argsort()[:1]]
        if not atm_calls.empty:
            atm_iv = atm_calls["iv"].values[0]

            smile_kpi1, smile_kpi2, smile_kpi3 = st.columns(3)
            smile_kpi1.metric("Vol implicite ATM", f"{atm_iv:.2%}")
            smile_kpi2.metric("Nb strikes (calls)", f"{len(calls_iv)}")
            smile_kpi3.metric("Nb strikes (puts)", f"{len(puts_iv)}")

        st.info(
            "Tu vois que le smile n'est PAS plat — contrairement à ce que prédit Black-Scholes. "
            "Les options OTM (loin du spot) ont une vol implicite plus élevée. "
            "C'est le 'volatility skew' : le marché price une probabilité de crash "
            "plus élevée que ce que la loi normale prédit."
        )